...
-- =====================================================
-- 1.5. CREAR TIPOS PERSONALIZADOS (ANTES DE USARLOS)
-- =====================================================
...
CREATE INDEX IF NOT EXISTS idx_recurring_transactions_next_payment ON recurring_transactions(next_payment_date);
...
CREATE TABLE IF NOT EXISTS budgets (
  ...
  alert_threshold SMALLINT DEFAULT 80 CHECK (alert_threshold >= 0 AND alert_threshold <= 100),
  is_active BOOLEAN DEFAULT TRUE,
  notes TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
...
CREATE INDEX IF NOT EXISTS idx_monthly_summary_month_year ON monthly_summary(month_year);
...
CREATE INDEX IF NOT EXISTS idx_payment_reminders_is_sent ON payment_reminders(is_sent);
...
CREATE INDEX IF NOT EXISTS idx_financial_recommendations_user_id ON financial_recommendations(user_id);
CREATE INDEX IF NOT EXISTS idx_financial_recommendations_type ON financial_recommendations(type);
CREATE INDEX IF NOT EXISTS idx_financial_recommendations_is_read ON financial_recommendations(is_read);
...
DROP TRIGGER IF EXISTS update_monthly_summary_updated_at ON monthly_summary;
...
DROP POLICY IF EXISTS users_see_own_transactions ON transactions;
...
DROP POLICY IF EXISTS users_see_own_goals ON savings_goals;
CREATE POLICY users_see_own_goals ON savings_goals
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS users_see_own_budgets ON budgets;
